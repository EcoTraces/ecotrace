...
const processTypes = ["materialSort", "dismantling", "componentSeparation"] as const;
const stageTransitions: Record<string, readonly string[]> = {
  created: ["sorting"],
  sorting: ["dismantling"],
  dismantling: ["componentSeparation"],
  componentSeparation: ["materialRecovery", "hazardousHandling"],
  materialRecovery: ["hazardousHandling", "finalDisposal", "verification"],
  hazardousHandling: ["materialRecovery", "finalDisposal", "verification"],
  finalDisposal: ["verification"],
  verification: [],
  completed: [],
};
...
function assertBatchOpen(snapshot: FirebaseFirestore.DocumentSnapshot, action: string) {
  if (snapshot.get("completionVerified") === true || snapshot.get("stage") === "completed") {
    throw new ApiError(409, `${action} is not allowed after batch completion.`, "invalid_state");
  }
}
...
router.post("/recycling/batches", authenticate, requireRoles(...operators), async (request, response) => {
  ...
  if (items.some((item) => !["recyclable", "hazardous", "nonRecoverable"].includes(String(item.get("condition") ?? "")))) throw new ApiError(409, "Only recyclable, hazardous, or non-recoverable items can enter a recycling batch.", "invalid_item_condition");
  if (items.some((item) => ["recycling", "recovered", "disposed"].includes(String(item.get("processingStatus") ?? "")))) throw new ApiError(409, "One or more items are already in a terminal or conflicting recycling state.", "invalid_item_state");
...
router.patch("/recycling/batches/:id/facility", authenticate, requireRoles(...operators), async (request, response) => {
  ...
  const batch = await batchRef(id(request.params.id));
  assertBatchOpen(batch.snapshot, "Facility reassignment");
  if (batch.snapshot.get("stage") !== "created") throw new ApiError(409, "The facility cannot change after processing begins.", "invalid_state");
...
router.patch("/recycling/batches/:id/stage", authenticate, requireRoles(...operators), async (request, response) => {
  ...
  const batch = await batchRef(id(request.params.id));
  assertBatchOpen(batch.snapshot, "Stage updates");
  const current = String(batch.snapshot.get("stage") ?? "created");
  if (input.stage === "completed") throw new ApiError(409, "Use completion verification to complete a batch.", "invalid_state");
  if (input.stage === current) throw new ApiError(409, "The batch is already at this processing stage.", "invalid_state");
  if (!(stageTransitions[current] ?? []).includes(input.stage)) throw new ApiError(409, `Cannot move a recycling batch from ${current} to ${input.stage}.`, "invalid_state");
  const write = db.batch();
  write.update(batch.ref, {stage: input.stage, ...(current === "created" ? {processingStartedAt: FieldValue.serverTimestamp()} : {}), updatedAt: FieldValue.serverTimestamp()});
...
router.post("/recycling/batches/:id/process-records", authenticate, requireRoles(...operators), async (request, response) => {
  const input = z.object({type: z.enum(processTypes), material: z.string().trim().max(200).default(""), component: z.string().trim().max(200).default(""), quantity: z.number().int().nonnegative(), weightKg: z.number().nonnegative(), notes: z.string().trim().min(1).max(2000)}).superRefine((value, context) => { if (value.type === "materialSort" && !value.material) context.addIssue({code: "custom", message: "Material is required for sorting records."}); if (value.type === "componentSeparation" && !value.component) context.addIssue({code: "custom", message: "Component is required for separation records."}); }).parse(request.body);
  const batch = await batchRef(id(request.params.id));
  assertBatchOpen(batch.snapshot, "Process records");
  const requiredStage = input.type === "materialSort" ? "sorting" : input.type;
  if (batch.snapshot.get("stage") !== requiredStage) throw new ApiError(409, `Record ${input.type} while the batch is in ${requiredStage}.`, "invalid_state");
...
router.post("/recycling/batches/:id/outputs", authenticate, requireRoles(...operators), async (request, response) => {
  ...
  await db.runTransaction(async (transaction) => {
    ...
    if (!snapshot.exists) throw new ApiError(404, "Recycling batch not found.", "not_found");
    assertBatchOpen(snapshot, "Recovered or hazardous outputs");
    if (!["materialRecovery", "hazardousHandling"].includes(String(snapshot.get("stage")))) throw new ApiError(409, "Outputs can only be recorded during material recovery or hazardous handling.", "invalid_state");
    ...
    transaction.update(ref, {[field]: FieldValue.increment(input.weightKg), updatedAt: FieldValue.serverTimestamp()});
  ...
  if (input.kind === "hazardous") void publishNotificationEvent({event: "recycling_hazardous_material_recorded", affectedUserIds: [request.user!.uid], data: {batchId: ref.id, material: input.material, weightKg: input.weightKg}});
...
router.post("/recycling/batches/:id/losses", authenticate, requireRoles(...operators), async (request, response) => {
  ...
  await db.runTransaction(async (transaction) => {
    const snapshot = await transaction.get(ref);
    if (!snapshot.exists) throw new ApiError(404, "Recycling batch not found.", "not_found");
    assertBatchOpen(snapshot, "Processing-loss records");
    if (!["dismantling", "componentSeparation", "materialRecovery", "hazardousHandling", "finalDisposal"].includes(String(snapshot.get("stage")))) throw new ApiError(409, "Processing loss cannot be recorded at the current stage.", "invalid_state");
...
router.post("/recycling/batches/:id/disposals", authenticate, requireRoles(...operators), async (request, response) => {
  ...
  await db.runTransaction(async (transaction) => {
    const snapshot = await transaction.get(ref);
    if (!snapshot.exists) throw new ApiError(404, "Recycling batch not found.", "not_found");
    assertBatchOpen(snapshot, "Final disposal records");
    if (snapshot.get("stage") !== "finalDisposal") throw new ApiError(409, "Move the batch to final disposal before recording disposal.", "invalid_state");
    ...
    transaction.update(ref, {disposedWeightKg: FieldValue.increment(input.weightKg), updatedAt: FieldValue.serverTimestamp()});
...
router.patch("/recycling/batches/:id/verify", authenticate, requireRoles(...supervisors), async (request, response) => {
  ...
  await db.runTransaction(async (transaction) => {
    const snapshot = await transaction.get(ref);
    if (!snapshot.exists) throw new ApiError(404, "Recycling batch not found.", "not_found");
    assertBatchOpen(snapshot, "Completion verification");
    if (snapshot.get("stage") !== "verification") throw new ApiError(409, "Move the batch to verification before completing it.", "invalid_state");
    ...
    transaction.update(ref, {stage: "completed", completionVerified: true, verificationNotes: input.notes, certificateNumber: `ECO-REC-${ref.id.toUpperCase()}`, certificateGeneratedAt: FieldValue.serverTimestamp(), verifiedBy: request.user!.uid, verifiedAt: FieldValue.serverTimestamp(), completedAt: FieldValue.serverTimestamp(), updatedAt: FieldValue.serverTimestamp()});
...
router.get("/recycling/reports/summary", authenticate, requireRoles(...operators), async (_request, response) => {
  ...
  const byFacility: Record<string, {batches: number; inputWeightKg: number; recoveredWeightKg: number; completedBatches: number}> = {};
  const totals = snapshot.docs.reduce((value, doc) => {
    const data = doc.data();
    const facility = String(data.facilityName ?? data.facilityId ?? "Unassigned");
    byFacility[facility] ??= {batches: 0, inputWeightKg: 0, recoveredWeightKg: 0, completedBatches: 0};
    byFacility[facility].batches++; byFacility[facility].inputWeightKg += number(data.inputWeightKg); byFacility[facility].recoveredWeightKg += number(data.recoveredWeightKg); if (data.completionVerified === true) byFacility[facility].completedBatches++;
  ...
  response.json({data: {...totals, recoveryEfficiencyPercent: totals.inputWeightKg ? totals.recoveredWeightKg / totals.inputWeightKg * 100 : 0, byFacility}});
});
