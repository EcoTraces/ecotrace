...
router.patch("/repairs/:id/progress", authenticate, requireRoles(...technicians), async (request, response) => {
  const input = z.object({progressPercent: z.number().int().min(0).max(95), details: z.string().trim().min(1).max(2000)}).parse(request.body); const job = await assertTechnicianAccess(id(request.params.id), request.user!.uid, request.user!.role); const current = Number(job.snapshot.get("progressPercent") ?? 0); if (input.progressPercent < current) throw new ApiError(409, "Progress cannot decrease.", "invalid_state"); const batch = db.batch(); batch.update(job.ref, {progressPercent: input.progressPercent, updatedAt: FieldValue.serverTimestamp()}); progress(batch, job.ref, "progressUpdated", input.details, input.progressPercent, request.user!.uid); await batch.commit(); response.json({data: {id: job.ref.id, progressPercent: input.progressPercent}});
});

router.patch("/repairs/:id/quality-testing", authenticate, requireRoles(...technicians), async (request, response) => { const job = await assertTechnicianAccess(id(request.params.id), request.user!.uid, request.user!.role); const batch = db.batch(); batch.update(job.ref, {status: "qualityTesting", progressPercent: 95, qualityTestingStartedAt: FieldValue.serverTimestamp(), updatedAt: FieldValue.serverTimestamp()}); progress(batch, job.ref, "qualityTestingStarted", "Repair submitted for quality control", 95, request.user!.uid); await batch.commit(); response.json({data: {id: job.ref.id, status: "qualityTesting"}}); });
router.patch("/repairs/:id/quality-control", authenticate, requireRoles(...technicians), async (request, response) => {
  const input = z.object({checks: z.record(z.string(), z.boolean()).refine((x) => Object.keys(x).length > 0), notes: z.string().trim().max(2000).default(""), grade: z.enum(grades), warrantyMonths: z.number().int().min(0).max(120)}).parse(request.body); const job = await assertTechnicianAccess(id(request.params.id), request.user!.uid, request.user!.role); const passed = Object.values(input.checks).every(Boolean); const start = new Date(); const end = new Date(start); end.setMonth(end.getMonth() + input.warrantyMonths); const batch = db.batch(); batch.update(job.ref, {status: passed ? "completed" : "repairInProgress", progressPercent: passed ? 100 : 80, qualityChecks: input.checks, qualityNotes: input.notes, qualityTestedBy: request.user!.uid, qualityTestedAt: FieldValue.serverTimestamp(), ...(passed ? {refurbishmentGrade: input.grade, warrantyStart: Timestamp.fromDate(start), warrantyEnd: Timestamp.fromDate(end), completedAt: FieldValue.serverTimestamp()} : {}), updatedAt: FieldValue.serverTimestamp()}); batch.update(db.collection("inventoryItems").doc(String(job.snapshot.get("itemId"))), {processingStatus: passed ? "refurbished" : "repairing", ...(passed ? {condition: "working"} : {}), updatedAt: FieldValue.serverTimestamp()}); progress(batch, job.ref, passed ? "qualityPassed" : "qualityFailed", input.notes, passed ? 100 : 80, request.user!.uid); await batch.commit(); response.json({data: {id: job.ref.id, passed, status: passed ? "completed" : "repairInProgress"}});
});

router.patch("/repairs/:id/unrepairable", authenticate, requireRoles(...technicians), async (request, response) => { const input = z.object({reason: z.string().trim().min(1).max(2000)}).parse(request.body); const job = await assertTechnicianAccess(id(request.params.id), request.user!.uid, request.user!.role); const batch = db.batch(); batch.update(job.ref, {status: "unrepairable", unrepairableReason: input.reason, unrepairableBy: request.user!.uid, closedAt: FieldValue.serverTimestamp(), updatedAt: FieldValue.serverTimestamp()}); batch.update(db.collection("inventoryItems").doc(String(job.snapshot.get("itemId"))), {processingStatus: "assignedForRecycling", condition: "recyclable", updatedAt: FieldValue.serverTimestamp()}); progress(batch, job.ref, "markedUnrepairable", input.reason, Number(job.snapshot.get("progressPercent") ?? 0), request.user!.uid); await batch.commit(); response.json({data: {id: job.ref.id, status: "unrepairable"}}); });
router.patch("/repairs/:id/warranty", authenticate, requireRoles(...supervisors), async (request, response) => { const input = z.object({warrantyEnd: z.coerce.date()}).parse(request.body); const job = await jobRef(id(request.params.id)); const batch = db.batch(); batch.update(job.ref, {warrantyEnd: Timestamp.fromDate(input.warrantyEnd), updatedAt: FieldValue.serverTimestamp()}); progress(batch, job.ref, "warrantyUpdated", `Warranty valid until ${input.warrantyEnd.toISOString()}`, Number(job.snapshot.get("progressPercent") ?? 0), request.user!.uid); await batch.commit(); response.json({data: {id: job.ref.id}}); });
router.patch("/repairs/:id/disposition", authenticate, requireRoles(...supervisors), async (request, response) => { const input = z.object({disposition: z.enum(["donation", "resale"]), resalePrice: z.number().nonnegative().nullable().optional(), donationRecipient: z.string().trim().max(300).default(""), notes: z.string().trim().max(2000).default("")}).superRefine((x, context) => { if (x.disposition === "resale" && x.resalePrice == null) context.addIssue({code: "custom", message: "Resale price is required."}); if (x.disposition === "donation" && !x.donationRecipient) context.addIssue({code: "custom", message: "Donation recipient is required."}); }).parse(request.body); const job = await jobRef(id(request.params.id)); if (job.snapshot.get("status") !== "completed") throw new ApiError(409, "Only completed repairs can be approved for donation or resale.", "invalid_state"); const batch = db.batch(); batch.update(job.ref, {disposition: input.disposition, dispositionApproved: true, dispositionApprovedBy: request.user!.uid, dispositionApprovedAt: FieldValue.serverTimestamp(), dispositionNotes: input.notes, resalePrice: input.disposition === "resale" ? input.resalePrice : null, donationRecipient: input.disposition === "donation" ? input.donationRecipient : "", updatedAt: FieldValue.serverTimestamp()}); progress(batch, job.ref, "dispositionApproved", `${input.disposition}: ${input.notes}`, 100, request.user!.uid); await batch.commit(); response.json({data: {id: job.ref.id, disposition: input.disposition}}); });
