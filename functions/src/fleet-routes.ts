...
router.patch("/vehicles/:id", authenticate, requireRoles(...managers), async (request, response) => {
  ...
  const vehicle = await vehicleRecord(id(request.params.id));
  if (input.driverId !== undefined) await validateDriver(input.driverId);
...
router.post("/vehicles/:id/maintenance", authenticate, requireRoles(...managers), async (request, response) => {
  const input = z.object({type: z.enum(["preventive", "corrective", "inspectionFollowUp", "service"]), description: z.string().trim().min(2).max(2000), scheduledAt: z.coerce.date(), estimatedCostSLE: z.number().nonnegative().default(0), provider: z.string().trim().max(200).default("")}).parse(request.body);
...
router.patch("/vehicles/:id/breakdowns/:recordId/resolve", authenticate, requireRoles(...managers), async (request, response) => { const input = z.object({resolution: z.string().trim().min(2).max(2000), repairCostSLE: z.number().nonnegative().default(0)}).parse(request.body); const vehicle = await vehicleRecord(id(request.params.id)); const record = vehicle.ref.collection("breakdowns").doc(id(request.params.recordId)); if (!(await record.get()).exists) throw new ApiError(404, "Breakdown record not found.", "not_found"); const batch = db.batch(); batch.update(record, {...input, status: "resolved", resolvedBy: request.user!.uid, resolvedAt: FieldValue.serverTimestamp(), updatedAt: FieldValue.serverTimestamp()}); batch.update(vehicle.ref, {availability: "available", updatedAt: FieldValue.serverTimestamp()}); batch.set(vehicle.ref.collection("events").doc(), {type: "breakdownResolved", details: input.resolution, recordId: record.id, actorId: request.user!.uid, createdAt: FieldValue.serverTimestamp()}); await batch.commit(); response.json({data: {id: record.id, status: "resolved"}}); });
