...
router.post("/partners", authenticate, requireRoles(...managers), async (request, response) => {
  const input = z.object({name: z.string().trim().min(2).max(300), registrationNumber: z.string().trim().min(2).max(100), contactName: z.string().trim().min(2).max(200), contactEmail: z.string().trim().email(), contactPhone: z.string().trim().min(5).max(50), address: z.string().trim().min(2).max(500), serviceCategories: z.array(z.enum(serviceCategories)).min(1), serviceAreas: z.array(z.string().trim().min(2).max(200)).min(1), facilityCapacityKg: z.number().nonnegative(), paymentCurrency: z.string().trim().length(3).default("SLE")}).parse(request.body);
  ...
  await ref.set({...input, partnerCode: `PTR-${ref.id.substring(0, 8).toUpperCase()}`, status: "pendingVerification", licenceVerified: false, complianceScore: 0, performanceRating: 0, ratingCount: 0, activeContractCount: 0, pricing: {}, paymentInformation: {}, createdBy: request.user!.uid, createdAt: FieldValue.serverTimestamp(), updatedAt: FieldValue.serverTimestamp()});
...
router.patch("/partners/:id/documents/:documentId/verify", authenticate, requireRoles(...managers), async (request, response) => {
  ...
  if (document.get("type") === "operatingLicence") batch.update(partner.ref, {licenceVerified: input.decision === "verified", status: input.decision === "verified" ? "active" : "verificationRejected", updatedAt: FieldValue.serverTimestamp()});
...
router.post("/partners/:id/contracts", authenticate, requireRoles(...managers), async (request, response) => {
  const input = z.object({title: z.string().trim().min(2).max(300), serviceCategories: z.array(z.enum(serviceCategories)).min(1), startsAt: z.coerce.date(), endsAt: z.coerce.date(), value: z.number().nonnegative(), currency: z.string().trim().length(3).default("SLE"), documentUrl: z.string().url(), paymentTerms: z.string().trim().min(2).max(1000)}).parse(request.body);
  ...
  batch.set(ref, {...input, startsAt: Timestamp.fromDate(input.startsAt), endsAt: Timestamp.fromDate(input.endsAt), status: "active", createdBy: request.user!.uid, createdAt: FieldValue.serverTimestamp(), updatedAt: FieldValue.serverTimestamp()});
...
router.patch("/partners/:id/commercial", authenticate, requireRoles(...managers), async (request, response) => {
  const input = z.object({pricing: z.record(z.string(), z.number().nonnegative()), paymentInformation: z.object({accountName: z.string().trim().min(2).max(200), bankName: z.string().trim().min(2).max(200), accountReference: z.string().trim().min(2).max(200), mobileMoneyProvider: z.string().trim().max(100).default(""), mobileMoneyNumber: z.string().trim().max(50).default("")})}).parse(request.body);
  const partner = await partnerRecord(id(request.params.id)); await partner.ref.update({...input, updatedBy: request.user!.uid, updatedAt: FieldValue.serverTimestamp()});
  response.json({data: {id: partner.ref.id}});
});
...
router.patch("/partners/:id/status", authenticate, requireRoles(...managers), async (request, response) => {
  ...
  const partner = await partnerRecord(id(request.params.id)); await partner.ref.update({status: input.status, statusReason: input.reason, statusChangedBy: request.user!.uid, statusChangedAt: FieldValue.serverTimestamp(), updatedAt: FieldValue.serverTimestamp()});
