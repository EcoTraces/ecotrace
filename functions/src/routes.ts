...
const centreSchema = z.object({
  ...
  supportedCategories: z.array(z.enum(["computers", "phones", "televisions", "appliances", "batteries", "accessories", "other"])).default([]),
...
const vehicleSchema = z.object({
  ...
  type: z.enum(["van", "truck", "pickupTruck", "motorcycle", "specializedHazardous"]),
  capacityKg: z.number().positive(),
...
const pickupSchema = z.object({
  category: z.enum(["computers", "phones", "televisions", "appliances", "batteries", "accessories", "other"]),
...
const dispatchOperators = ["administrator", "superAdministrator", "collector", "driver"] as const;

router.get("/me", authenticate, async (request, response) => {
  const profile = await db.collection("users").doc(request.user!.uid).get();
  response.json({data: {uid: request.user!.uid, email: request.user!.email, role: request.user!.role, profile: profile.exists ? documentJson(profile.id, profile.data()!) : null}});
});
...
router.get("/collection-centres", async (_request, response) => {
  const snapshot = await db.collection("collectionCentres").where("active", "==", true).get();
...
router.post(
  ...
  async (request, response) => {
    ...
    batch.set(reference.collection("events").doc(), {type: "registered", details: "Vehicle registered through API", createdAt: FieldValue.serverTimestamp(), actorId: request.user!.uid});
    await batch.commit();
    response.status(201).json({data: {id: reference.id}});
  },
);
...
router.get("/pickup-requests", authenticate, async (request, response) => {
  const privileged = ["administrator", "superAdministrator", "collector", "driver"].includes(request.user!.role);
  const query = privileged ? db.collection("pickupRequests") : db.collection("pickupRequests").where("userId", "==", request.user!.uid);
...
router.post("/pickup-requests", authenticate, async (request, response) => {
  ...
  const fee = (input.quantity * 2) + (input.estimatedWeight * 0.75) + (input.urgent ? 15 : 0);
...
router.patch("/pickup-requests/:id/cancel", authenticate, async (request, response) => {
  const reference = await requirePickupAccess(routeParameter(request.params.id), request.user!.uid, request.user!.role);
  await reference.update({status: "cancelled", updatedAt: FieldValue.serverTimestamp()});
...
router.patch("/pickup-requests/:id/reschedule", authenticate, async (request, response) => {
  const input = z.object({scheduledAt: z.coerce.date()}).parse(request.body);
  const reference = await requirePickupAccess(routeParameter(request.params.id), request.user!.uid, request.user!.role);
  await reference.update({scheduledAt: Timestamp.fromDate(input.scheduledAt), status: "submitted", updatedAt: FieldValue.serverTimestamp()});
...
router.patch("/pickup-requests/:id/rating", authenticate, async (request, response) => {
  const input = z.object({rating: z.number().int().min(1).max(5)}).parse(request.body);
  const reference = await requirePickupAccess(routeParameter(request.params.id), request.user!.uid, request.user!.role);
  await reference.update({rating: input.rating, ratedAt: FieldValue.serverTimestamp(), updatedAt: FieldValue.serverTimestamp()});
...
router.get(
  ...
  async (request, response) => {
    ...
    let query: FirebaseFirestore.Query = db.collection("collectionSchedules")
...
router.get(
  ...
  requireRoles(...dispatchSupervisors),
  async (_request, response) => {
    const snapshot = await db.collection("users").where("role", "in", ["collector", "driver"]).get();
    response.json({data: snapshot.docs.map((document) => documentJson(document.id, {
      displayName: document.get("displayName") ?? document.get("email") ?? document.id,
...
router.post(
  ...
  async (request, response) => {
    ...
    const pickupRefs = input.pickupIds.map((id) => db.collection("pickupRequests").doc(id));
    const [vehicle, ...pickups] = await db.getAll(vehicleRef, ...pickupRefs);
    if (!vehicle.exists || vehicle.get("availability") !== "available") {
      throw new (await import("./errors.js")).ApiError(409, "The selected vehicle is not available.", "vehicle_unavailable");
    }
    if (pickups.some((pickup) => !pickup.exists || !["submitted", "approved"].includes(String(pickup.get("status"))))) {
      throw new (await import("./errors.js")).ApiError(409, "One or more pickups are no longer assignable.", "pickup_unavailable");
    }
    const estimatedWeight = pickups.reduce((total, pickup) => total + Number(pickup.get("estimatedWeight") ?? 0), 0);
    if (estimatedWeight > Number(vehicle.get("capacityKg") ?? 0)) {
      throw new (await import("./errors.js")).ApiError(409, "Selected pickups exceed vehicle capacity.", "capacity_exceeded");
    }
...
async function scheduleForAction(id: string, uid: string, role: string) {
  ...
  const snapshot = await reference.get();
  if (!snapshot.exists) {
    throw new (await import("./errors.js")).ApiError(404, "Collection schedule not found.", "not_found");
  }
  const supervisor = dispatchSupervisors.some((candidate) => candidate === role);
  const assigned = snapshot.get("driverId") === uid || (snapshot.get("collectorIds") ?? []).includes(uid);
  if (!supervisor && !assigned) {
    throw new (await import("./errors.js")).ApiError(403, "You are not assigned to this collection.", "forbidden");
  }
  return {reference, snapshot};
}

router.patch("/dispatch/schedules/:id/dispatch", authenticate, requireRoles(...dispatchSupervisors), async (request, response) => {
  const {reference, snapshot} = await scheduleForAction(routeParameter(request.params.id), request.user!.uid, request.user!.role);
  const batch = db.batch();
  batch.update(reference, {status: "dispatched", dispatchedAt: FieldValue.serverTimestamp(), updatedAt: FieldValue.serverTimestamp()});
  batch.update(db.collection("vehicles").doc(snapshot.get("vehicleId")), {availability: "dispatched", updatedAt: FieldValue.serverTimestamp()});
  for (const pickupId of snapshot.get("pickupIds") ?? []) batch.update(db.collection("pickupRequests").doc(pickupId), {status: "scheduled", updatedAt: FieldValue.serverTimestamp()});
  await batch.commit();
  response.json({data: {id: reference.id, status: "dispatched"}});
});

router.patch("/dispatch/schedules/:id/start", authenticate, requireRoles(...dispatchOperators), async (request, response) => {
  const {reference, snapshot} = await scheduleForAction(routeParameter(request.params.id), request.user!.uid, request.user!.role);
  const batch = db.batch();
...
router.patch("/dispatch/schedules/:id/complete", authenticate, requireRoles(...dispatchOperators), async (request, response) => {
  const input = z.object({evidenceUrls: z.array(z.string().url()).default([])}).parse(request.body);
  const {reference, snapshot} = await scheduleForAction(routeParameter(request.params.id), request.user!.uid, request.user!.role);
  const batch = db.batch();
  batch.update(reference, {status: "completed", completedAt: FieldValue.serverTimestamp(), evidenceUrls: input.evidenceUrls, updatedAt: FieldValue.serverTimestamp()});
  const vehicle = db.collection("vehicles").doc(snapshot.get("vehicleId"));
  batch.update(vehicle, {availability: "available", updatedAt: FieldValue.serverTimestamp()});
...
router.patch("/dispatch/schedules/:id/reschedule", authenticate, requireRoles(...dispatchSupervisors), async (request, response) => {
  const input = z.object({scheduledAt: z.coerce.date()}).parse(request.body);
  const {reference, snapshot} = await scheduleForAction(routeParameter(request.params.id), request.user!.uid, request.user!.role);
  const batch = db.batch();
  batch.update(reference, {status: "planned", scheduledAt: Timestamp.fromDate(input.scheduledAt), missedAt: FieldValue.serverTimestamp(), updatedAt: FieldValue.serverTimestamp()});
  batch.update(db.collection("vehicles").doc(snapshot.get("vehicleId")), {availability: "available", updatedAt: FieldValue.serverTimestamp()});
  for (const pickupId of snapshot.get("pickupIds") ?? []) batch.update(db.collection("pickupRequests").doc(pickupId), {status: "assigned", scheduledAt: Timestamp.fromDate(input.scheduledAt), updatedAt: FieldValue.serverTimestamp()});
  await batch.commit();
