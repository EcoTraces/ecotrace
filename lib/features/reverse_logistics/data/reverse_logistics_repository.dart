...
class ReverseLogisticsRepository {
  ...
  Stream<List<ReverseLogisticsTransfer>> watchTransfers() =>
      _transfers.snapshots().map(
        (snapshot) =>
            snapshot.docs.map(ReverseLogisticsTransfer.fromDoc).toList()..sort(
  ...
  Stream<ReverseLogisticsTransfer> watchTransfer(String id) =>
      _transfers.doc(id).snapshots().map(ReverseLogisticsTransfer.fromDoc);

  Stream<List<CustodyEvent>> watchCustody(String id) => _transfers
      .doc(id)
      ...
      .map(
        (snapshot) => snapshot.docs.map(CustodyEvent.fromDoc).toList()
          ..sort(
            (a, b) =>
                (b.at ?? DateTime(2000)).compareTo(a.at ?? DateTime(2000)),
          ),
      );

  Stream<List<TransferException>> watchExceptions(String id) => _transfers
      .doc(id)
  ...
  Stream<List<UserProfile>> watchDrivers() => _db
      .collection('users')
      .where('role', isEqualTo: 'driver')
      .snapshots()
      .map((snapshot) => snapshot.docs.map(UserProfile.fromSnapshot).toList());

  Stream<List<CollectionCentre>> watchCentres() => _db
      .collection('collectionCentres')
      .where('active', isEqualTo: true)
      .snapshots()
      .map((snapshot) => snapshot.docs.map(CollectionCentre.fromDoc).toList());
