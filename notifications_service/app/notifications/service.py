...
EVENTS: dict[EventName, EventTemplate] = {
    ...
    EventName.CENTRE_STAFF_ASSIGNED: EventTemplate(NotificationType.ADMIN_MESSAGE, "Collection Centre Assignment", "You have been assigned to a collection centre."),
    EventName.CENTRE_CAPACITY_ALERT: EventTemplate(NotificationType.ADMIN_MESSAGE, "Collection Centre Capacity Alert", "A collection centre has reached its configured storage threshold.", True),
    EventName.CENTRE_SAFETY_ALERT: EventTemplate(NotificationType.ADMIN_MESSAGE, "Collection Centre Safety Alert", "A collection centre safety inspection requires attention.", True),
    EventName.REPAIR_TECHNICIAN_ASSIGNED: EventTemplate(NotificationType.ADMIN_MESSAGE, "Repair Job Assigned", "A repair job has been assigned to you."),
    EventName.REPAIR_REVIEWED: EventTemplate(NotificationType.ADMIN_MESSAGE, "Repair Assessment Reviewed", "A repair assessment has been approved or rejected."),
    EventName.REPAIR_COMPLETED: EventTemplate(NotificationType.ADMIN_MESSAGE, "Repair Completed", "A repaired device passed quality control and entered refurbished inventory."),
    EventName.REPAIR_UNREPAIRABLE: EventTemplate(NotificationType.ADMIN_MESSAGE, "Item Marked Unrepairable", "An item was marked unrepairable and routed for recycling."),
    EventName.REPAIR_DISPOSITION_APPROVED: EventTemplate(NotificationType.ADMIN_MESSAGE, "Refurbished Device Disposition", "A refurbished device was approved for donation or resale."),
    EventName.RECYCLING_HAZARDOUS_MATERIAL_RECORDED: EventTemplate(NotificationType.ADMIN_MESSAGE, "Hazardous Recycling Output", "Hazardous material was separated from a recycling batch and requires controlled handling.", True),
    EventName.HAZARDOUS_INCIDENT_REPORTED: EventTemplate(NotificationType.ADMIN_MESSAGE, "Hazardous Waste Emergency", "A hazardous-waste incident requires an immediate response.", True),
    EventName.HAZARDOUS_EMERGENCY_RESPONSE_UPDATED: EventTemplate(NotificationType.ADMIN_MESSAGE, "Emergency Response Updated", "A hazardous-waste incident response has been updated.", True),
    EventName.HAZARDOUS_INCIDENT_CLOSED: EventTemplate(NotificationType.ADMIN_MESSAGE, "Hazardous Incident Closed", "A hazardous-waste incident has been closed.", True),
    EventName.HAZARDOUS_WASTE_CERTIFIED: EventTemplate(NotificationType.ADMIN_MESSAGE, "Hazardous Disposal Certified", "A hazardous-waste disposal certificate has been issued.", True),
    EventName.LOGISTICS_TRANSFER_SUBMITTED: EventTemplate(NotificationType.ADMIN_MESSAGE, "Transfer Approval Required", "A reverse-logistics transfer is awaiting approval.", True),
    EventName.LOGISTICS_TRANSFER_REVIEWED: EventTemplate(NotificationType.ADMIN_MESSAGE, "Transfer Reviewed", "Your reverse-logistics transfer was reviewed."),
    EventName.LOGISTICS_TRANSPORT_ASSIGNED: EventTemplate(NotificationType.ADMIN_MESSAGE, "Transfer Assigned", "You were assigned to a reverse-logistics transfer."),
    EventName.LOGISTICS_DISPATCHED: EventTemplate(NotificationType.ADMIN_MESSAGE, "Transfer Dispatched", "Your reverse-logistics transfer is ready for transit."),
    EventName.LOGISTICS_EXCEPTION_REPORTED: EventTemplate(NotificationType.ADMIN_MESSAGE, "Logistics Exception", "A reverse-logistics exception requires attention.", True),
    EventName.LOGISTICS_RECEIVED: EventTemplate(NotificationType.ADMIN_MESSAGE, "Transfer Received", "A reverse-logistics transfer was received."),
    EventName.PARTNER_LICENCE_REVIEWED: EventTemplate(NotificationType.COMPLIANCE_ALERT, "Partner Licence Reviewed", "A partner licence decision was recorded.", administrators=True),
    EventName.PARTNER_STATUS_CHANGED: EventTemplate(NotificationType.COMPLIANCE_ALERT, "Partner Status Changed", "A partner status was changed.", administrators=True),
    EventName.MARKETPLACE_DELIVERY_UPDATED: EventTemplate(NotificationType.STATUS_UPDATE, "Marketplace Delivery Update", "Your marketplace delivery status changed."),
    EventName.DONATION_REQUEST_SUBMITTED: EventTemplate(NotificationType.ADMIN_MESSAGE, "Donation Request Submitted", "A new donation request requires review.", administrators=True),
    EventName.DONATION_STATUS_UPDATED: EventTemplate(NotificationType.STATUS_UPDATE, "Donation Status Updated", "Your donation request status changed."),
